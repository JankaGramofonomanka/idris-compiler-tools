...
namespace Graph
  ...
  namespace Mixed
    ...
    mutual
      ...
      prepend v Empty = Empty |-| SingleVertex v |-| Empty

      prepend {lgins = Nil, rgins = Nil} v (SingleVertex {vins = Nothing} w)
        = rewrite concat_nil (fromVIn vins lbl)
       in SingleVertex (cnct @{impl} v w)

      prepend v (Cycle {loopIns, loopOuts} node loop) = let
        node' : CFG vertex (lgins ++ Undefined lbl :: (rgins ++ loopOuts)) (loopIns ++ gouts)
        node' = rewrite concat_assoc lgins (Undefined lbl :: rgins) loopOuts
             in node

        node'' : CFG vertex ((lgins ++ fromVIn vins lbl ++ rgins) ++ loopOuts) (loopIns ++ gouts)
        node'' = rewrite revEq $ concat_assoc lgins (fromVIn vins lbl ++ rgins) loopOuts
              in rewrite revEq $ concat_assoc (fromVIn vins lbl) rgins loopOuts
              in Mixed.prepend v node'
        in Cycle node'' loop
